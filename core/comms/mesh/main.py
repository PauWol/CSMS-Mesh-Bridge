...
from ..constants import (MAX_NEIGHBORS, MESH_TYPE_HELLO, MESH_TYPE_HELLO_ACK,
                         ...
                         PMK_BYTE_LEN, PMK_DEFAULT_KEY, MESH_FLAG_GATEWAY
                         )
...
class Mesh: # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self,auto_init: bool = False):
        ...
        :param auto_init: Runs Mesh.start function to init and activate mesh automatically.
        """
    ...
    def _requires_start(self):
        if self._esp == None:
            raise RuntimeError("Error: esp instance is None! use start function to initiate first.")
...
def mesh(auto_init: bool = False) -> Mesh:
    """
    This returns the Mesh instance.
    :param auto_init: Only used if no previous mesh instance exists. Uses start function to init and activate.
    :return:
    """
    global _mesh
    if _mesh is None:
        _mesh = Mesh(auto_init)
    return _mesh
